...
cv.svy <- function(Data, formulae, nfolds=5, strataID = NULL, clusterID = NULL , nest = FALSE, N, method = "linear", weights = NULL, afolds = TRUE) {
  ...
  for (fold in 1:nfolds) {
    ...
    test <- Data[test.rows,]
    n = nrow(train)
      gen.svy <- svydesign(ids = if(is.null(clusterID)) formula(~0) else formula(paste0("~", clusterID)),
  ...
  Data <- cbind(Data, test_errors_sq.df)
  ...
  while (i <= ncol(Data)) {
    meansd <- data.frame(svymean(Data[,i],gen.svy))
