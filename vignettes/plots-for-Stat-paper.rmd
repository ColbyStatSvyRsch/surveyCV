...
* TODO: Save the actual results of the CV-folds sims, so we can knit vignette quickly without `cache=TRUE` which isn't working for some reason
...
splinepop.df$Stratum <- cut(splinepop.df$Stratum, 5, 1:5)
splinepop.df$Cluster <- cut(splinepop.df$Cluster, 100, 1:100)
splinepop.df <- splinepop.df %>%
  dplyr::arrange(ID) %>%
...
```{r WeightsSimsResultsAndPlots, results = FALSE, fig.width=7, fig.height=7}
...
```{r FoldsSimsResults, results = FALSE, eval=FALSE}
...
for (i in 1:loops) {
  ...
  clus.CV.out <- cv.svy(sim.clus, modelsToFit,
                        clusterID = "Cluster", nfolds = 5) %>% as.vector()
...
for (i in 1:loops) {
  ...
  strat.CV.out <- cv.svy(sim.strat, modelsToFit,
                         strataID = "Stratum", nfolds = 5) %>% as.vector()
...
```{r FoldsSimsPlots, fig.width=7, fig.height=7, eval=FALSE}
