...
Based on the sims from `plot_generation.R` -- but instead of having similar functions repeatedly defined separately as in that file, use our functions such as `cv.svy()` directly from the package.
...
#   AND the 4 below, AND the 3 lists from early-results vignette,
#   all into R/sysdata.Rda together
```
...
# ...
# OK, now we've written those dataframes,
#   AND the 8 above, AND the 3 lists from early-results vignette,
#   all into R/sysdata.Rda together
```
