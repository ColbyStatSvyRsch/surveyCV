...
# Write a function that is a copy of code from inside of 
# survey:::extractAIC.svyglm
# so that we can skip the if(is.svylm(fit))
# which seems to be buggy, because it makes the dAICs look drastically different
# when we rescale the response variable...
dAIC <- function(fit) {
  ftest <- delete.response(formula(fit))
  r <- regTermTest(fit, ftest, method="LRT")
  d <- fit$deviance
  AIC <- d + 2 * sum(r$lambda)
  return(AIC)
}
# However, I'm still not convinced that this is right;
# we should probably remove the dAIC sims from the paper
# until we can confirm dAIC is working the way we expect.
...
for (i in 1:loops) {
  ...
  srs.dAIC.out <- sapply(1:6,
    function(ii) dAIC(svyglm(modelsToFit[ii], srs.des)))
...
for (i in 1:loops) {
  ...
  clus.dAIC.out <- sapply(1:6,
    function(ii) dAIC(svyglm(modelsToFit[ii], clus.des)))
...
for (i in 1:loops) {
  ...
  strat.dAIC.out <- sapply(1:6,
    function(ii) dAIC(svyglm(modelsToFit[ii], strat.des)))
